# This is the full cleaning and plotting script for the Santee Cooper 2026 report
...
live_dets1 <- read.csv("I:/Shared drives/IFI/Projects_Active/SanteeCooper_FERC_19-04-09/IFI_TASKS/Task_Y_SturgUseofLock/Data/Fathom_Downloads/Fathom_Live_July25-April26.csv")
live_dets2 <- read.csv("I:/Shared drives/IFI/Projects_Active/SanteeCooper_FERC_19-04-09/IFI_TASKS/Task_Y_SturgUseofLock/Data/Fathom_Downloads/Fathom_live_detections_april-June_2026.csv")
...
# --- Assumptions & Notes ---
# Receiver serial numbers are manually mapped to receiver locations; this mapping is based on 2025–2026 deployments.
...
# Season logic is custom: December 2025 is treated as Spring because fish behavior matches Spring patterns.
# Lock status (broken vs operational) is hard‑coded based on known outage dates (Feb 10–Apr 10, 2026).
...
# no tag is seen in fall 2025 AND spring 2026
...
# write.csv(observed.sturgeon,"I:/Shared drives/IFI/Projects_Active/SanteeCooper_FERC_19-04-09/IFI_TASKS/Task_Y_SturgUseofLock/Data/Figures_Working/2025 - 2026 Report/observed_sturgeon_2025_2026.csv",row.names = FALSE)


############   Figures         #################################################

# --- Assumptions & Notes ---
# All figures save to a hard‑coded path tied to the 2025–2026 report.
# Histograms use daily binwidth (24 hours) to match reporting requirements.
# Vertical red lines indicate lock outage period (Feb 10–Apr 10, 2026).
...
for (season in seasons) {
  for (id in ids) {
    ...
    for (page in seq_along(date_chunks)) {
      ...
      panel_df <- df_pts_page %>%
        distinct(date, lock.status) %>% 
        mutate(fill_col = ifelse(lock.status == "non operational", "#f4b6b6", "white"))
      panel_df$date <- factor(panel_df$date, levels = levels(df_pts_page$date))
      ...
      p <- ggplot(df_pts_page, aes(x = hour_of_day, y = forcats::fct_rev(receiver_location2), color = receiver_location2)) +
        ...
        scale_fill_manual(values = c("non operational" = "#f4b6b6", "operational" = "white")) +
